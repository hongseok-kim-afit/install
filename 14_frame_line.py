...
def check_inroute(pose_data, current_goal, point):
    ...
    # eliminate out of x 
    if point[0] > max(pose_data.translation.x, current_goal[0]) + noaction_distance or point[0] <  min(pose_data.translation.x, current_goal[0]) - noaction_distance:
        ...
        print('elim x')
        return 'out_route'
    elif point[2] > max(-pose_data.translation.z, current_goal[2]) + noaction_distance or point[2] <  min(-pose_data.translation.z, current_goal[2]):
...
try:
    ...
    while True:
        ...
        #grab_line1 = depth_meter[:,vertical_center-20]
        #grab_line2 = depth_meter[:,vertical_center+20]

        #print('new axis',grab_line)
        # grab the depth is less then 3000, longer than 500
        depth_line = np.where((grab_line<1.8) & (grab_line>0.5))
        #depth_line1 = np.where((grab_line1<3.000) & (grab_line1>.500))
        #depth_line2 = np.where((grab_line2<3.000) & (grab_line2>.500))

        #print('depth_line[0]',depth_line[0])
        x1 = depth_line[0][0] # x
        w1 = len(depth_line[0])

        grab_Vline = depth_meter[x1, :]
        depth_Vline = np.where((grab_Vline<1.8) & (grab_Vline> 0.5))
        
        y1 = depth_Vline[0][0]
        h1 = len(depth_Vline[0])

        #right_object = depth_line[0][0] + len(depth_line[0])

        depth_pro = grab_line[depth_line].astype(float)
        
        print('grab',depth_pro)
        # print('depth_line',depth_line)
        ...
        for contour in c:
            ...
            ################################################################################3
            #cv2.rectangle(depth_data, (x, y), (x + w, y + h), (0, 255, 0), 3) # draw target rectangle 
            text = "Depth: " + str("{0:.2f}").format(dist1)
            ...
            #target_depth * width / 4 = set the clearance 0.4m 
            #target_depth * width / 2 = set the clearance 1m 
            pixel_avoid_right = [pixel_right[0] + 1 / target_depth * width / 4, y] 
            pixel_avoid_left = [pixel_left[0] - 1 / target_depth * width / 4, y]

            # coordinate, the 0.4m out of the target edge 
            coordi_avoid_left = coordi_left - np.array([0.4, 0, 0])
            coordi_avoid_right = coordi_right + np.array([0.4, 0, 0])
        ...
        # if mode == 'HOLD':
            ...
            if check_inroute(pose_data, current_goal, coordi_edge)=='out_route' and check_inroute(pose_data, current_goal, pre_coordi_edge)=='out_route':
            ...
            # object_edge or pre_object_edge in route, avoid obejct
            else :
                if check_inroute(pose_data, current_goal, coordi_edge)=='out_route' and check_inroute(pose_data, current_goal, pre_coordi_edge)=='in_route':
                    ...
                    if noaction_distance < target_depth < desired_distance + distance_bound:
                        ...
                        print('avoid','pixel_center',pixel_center,'pixel_edge', pixel_edge,'depth' ,target_depth,'position' ,pose_data.translation,'coordi_edge', coordi_edge)
