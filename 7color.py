...
desired_distance = 2 #distance from desired action for object avoidance in meters
...
center_dist_bound = 10 #100 #last tested #pixels from center, defines x-y position tolerance
...
##############################################
def change_coordinate(pose_data):
    """
...
###########################################3
## movement function 
def object_avoid(difference, target_depth):
    '''
    difference = frame_center[0] - target_center[0]
    '''
    # target located left of frame_center 
    if 0 < difference < head_offset - center_dist_bound: 
        print('turn_right')
        set_rc_channel_pwm(right_turn,set_speed)
        set_rc_channel_pwm(right_turn,set_speed)

    # target located right of frame_center
    #elif 0 < -difference < head_offset - center_dist_bound: 
    else : 
        print('turn_left')
    ...
    '''
        ...
        if target_depth > 2:
            if head_offset < difference:
                print('reverse_left')
                set_rc_channel_pwm(left_turn + turn_min,set_speed)
            else:
                print('reverse_right')
                set_rc_channel_pwm(right_turn + turn_min,set_speed)
...
try:
    ...
    while True:
        ...
        # l_b = np.array([204, 102, 0]) # set hsv min color # color orange
        # u_b = np.array([255, 204, 153]) # set hsv max color # color orange
        # https://www.rapidtables.com/web/color/RGB_Color.html

        l_b = np.array([24, 133, 48]) # set hsv min color # color green
        u_b = np.array([39, 200, 181]) # set hsv max color # color green
        ...
        color = cv2.bitwise_and(color_image, color_image, mask=mask)  # only capture from l_b to u_b ## green color capture
        ...
        for contour in c:
            ...
            avoid_right_pixel = [target_right_edge + 1 / target_depth * width / 2, y]
            avoid_left_pixel = [target_left_edge - 1 / target_depth * width / 2, y]
            ...
            if mode == 'MANUAL':
                ...
                # object avoidance
                    ...
                    if noaction_distance < target_depth < 3.0:
                        ...
                        if desired_distance + distance_bound > target_depth :
        ...
        small_data = {
            ...
            'goal' : current_goal,
            'object' : [tx,ty,tz, target_depth]
            #'angle':[pose_data.rotation.w, pose_data.rotation.x, pose_data.rotation.y, pose_data.rotation.z],
            #'pixel_location': object_edge
            }
