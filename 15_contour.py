...
time_path = './data/'+str(st_goal)+str(t)
...
try:
    ...
    while True:
        ...
        for contour in c:
            ...
            pixel_avoid_right = [pixel_right[0] + 1 / target_depth * width / 3, pixel_right[1]] 
            pixel_avoid_left = [pixel_left[0] - 1 / target_depth * width / 3, pixel_left[1]]

            # coordinate, the 0.6m out of the target edge 
            ...
            if mode == 'MANUAL':
        2802259092141, -0.03294165650450706, 0.28641775147998816) coordi_right
(0.303        #print(pose_data.translation)
        ...
        # if mode == 'HOLD':
            ...
            # object_edge out route, pre_object_edge in route, avoid pre_target
                ...
                if pre_dist < desired_distance + distance_bound:
                    print('pre_coordi=inroute avoid')
                    relative_avoid(pose_data, pre_coordi_avoid, goal, ii)
                else:
                    ii = route_move (pose_data, goal, ii)
            
            # object_edge, and pre_object_edge in route, avoid both
            elif check_inroute(pose_data, current_goal, coordi_edge)=='in_route' and check_inroute(pose_data, current_goal, pre_coordi_edge)=='in_route':
                pre_angle, pre_dist, pre_yaw = relative_position(pose_data, pre_coordi_edge)

                if pre_dist < target_depth < desired_distance + distance_bound:
                    print('pre_coordi=inroute avoid')
                    relative_avoid(pose_data, pre_coordi_avoid, goal, ii)

                elif target_depth < pre_dist < desired_distance + distance_bound:
                    ox, oy, oz = depth_to_tracking (pixel_edge, target_depth, pose_data, width)
                    tx, ty, tz = depth_to_tracking (pixel_center, target_depth, pose_data, width)
